...
{
	/*GameObject* objA = new GameObject("objA");
	GameObject* child = objA;
	delete objA;
	objA = nullptr;
	std::cout << child->name;
	delete child;*/
	...
	GameObject* objtemp = objA->clone();

	delete objA;
	std::cout << objtemp->name;
	delete objtemp;
*/
	AceParse* ace = new AceParse();
	ace->parse("Resources/folder/test1.ace");
...
// 
	...
	Shader skyBoxShader("skybox2.vertex", "skybox.frag");
